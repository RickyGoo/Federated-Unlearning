    ...
    def _local_update(self,dataloader, local_ep, lr,optim_choice, ul_mode_train='none'):
        ...
        for epoch in range(local_ep):
            ...
            for batch_idx, (x, y) in enumerate(train_ldr):
                ...
                # print(pred.shape)
                # print(y.shape)
                loss += F.cross_entropy(pred, y.type(torch.int64))
        ...
        for epoch in range(local_ep):
            ...
            for batch_idx, (x, y) in enumerate(train_ldr):
                ...
                if 'ul_samples' in self.ul_mode:   #self.ul_mode=='ul_samples' or self.ul_mode=='ul_samples_backdoor' or 'u_samples_whole_client: # random false labels
                    ...
                    for label in y:
                        if label < (self.num_classes):
                            one_hot_label=torch.nn.functional.one_hot(label.to(torch.int64), self.num_classes).unsqueeze(0).to(self.device, dtype=torch.int64)
                            one_hot_label=torch.cat((one_hot_label,one_hot_label),dim=1)
                            
                        else:
                            true_label= label % (self.num_classes)
                            label_a=torch.nn.functional.one_hot(true_label.to(torch.int64), self.num_classes).unsqueeze(0).to(self.device, dtype=torch.int64)
                            ...
                            label_b=torch.nn.functional.one_hot(random_label.to(torch.int64), self.num_classes).unsqueeze(0).to(self.device, dtype=torch.int64)
                    ...
                    for label in y:
                        ...
                        one_hot_label_true=torch.nn.functional.one_hot(label.to(torch.int64), self.num_classes).unsqueeze(0).to(self.device, dtype=torch.int64)
    ...
    def test(self, dataloader):
        ...
        with torch.no_grad():
            for load in dataloader:
                ...
                # print(target)
                loss_meter += F.cross_entropy(pred, target.type(torch.int64), reduction='sum').item() #sum up batch loss
